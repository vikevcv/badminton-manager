...
public class BookingPanel extends JPanel {
    ...
    // State Constants: Các trạng thái của ô lưới, dùng để điều khiển màu sắc và
    // chặn thao tác click.
    ...
    // Timeline Generation: Tự động chia cột theo bước nhảy 30 phút (6:00 -> 22:00)
    // bằng vòng lặp while.
    private void setupTimelineTable() {
        ...
        tableModel = new DefaultTableModel(timeSlots.toArray(), 0) {// tạo bản có tên cột là timeSlot.toArray(), 0 hàng
                                                                    // mặc định
            @Override
        ...
        // Cell Coloring Logic: "Bộ não" đồ họa - Tự động tô màu (Đỏ, Xanh, Xám) và đổi
        // chữ hiển thị dựa trên giá trị của ô.
    ...
    public void addLoadSpinner(ChangeListener listener) {
    ...
    // Time-Sync Logic: Nạp dữ liệu sân, kiểm tra lịch bảo trì và đặc biệt là khóa
    // các ô thời gian đã qua so với giờ hiện tại.
    public void updateSchedule(List<Court> courts, List<Booking> bookings) {
        ...
        for (int i = 0; i < courts.size(); i++) {
            ...
            boolean isFullBooked = court.getStatus() != null && court.getStatus().name().equals("DA_DAT");
            for (int j = 1; j < rowData.length; j++) {
                if (isMaintenance) {
                    rowData[j] = MAINTENANCE;
                } else if (isFullBooked) {
                    rowData[j] = BOOKED; // FULL đỏ luôn
                } else {
            ...
            for (Booking b : bookings) {
                ...
                if (rowIndex != -1) {
                    ...
                    for (int c = startCol; c < endCol; c++) {
                        ...
                        if (c < tableModel.getColumnCount())
                            tableModel.setValueAt(BOOKED, rowIndex, c);
                    }
                }
            }
        }
    ...
    // Data Conversion: Thuật toán quét các ô màu xanh (SELECTING) để gộp lại thành
    // các đối tượng Booking gửi về Controller lưu xuống DB.
    public List<Booking> extractSelectedBookings() {
        ...
        for (int i = 0; i < tableModel.getRowCount(); i++) {
            ...
            for (int j = 1; j <= tableModel.getColumnCount(); j++) {
                boolean isSelecting = (j < tableModel.getColumnCount())
                        && SELECTING.equals(tableModel.getValueAt(i, j));
                if (isSelecting) {
                    if (startSlotCol == -1)
                        startSlotCol = j;
                    endSlotCol = j;
